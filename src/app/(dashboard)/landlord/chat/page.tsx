...
type ViewingRequest = Database['public']['Tables']['viewing_requests']['Row'] & {
    ...
    tenant?: { id: string; full_name: string; email: string; phone: string | null }
}

function LandlordChatContent() {
    ...
    if (viewingRequests.length === 0) {
        return (
            <div className="max-w-4xl mx-auto py-12 px-4 text-center space-y-4">
                ...
                <h1 className="text-xl font-black text-charcoal-tone">No Inquiries Found</h1>
                <p className="text-xs font-medium text-stone-slate max-w-sm mx-auto">Active message threads will emerge here as soon as tenants express inspection interest against your listed units.</p>
                <button
    ...
    return (
        <div className="h-[calc(100vh-112px)] flex flex-col bg-pure-white text-charcoal-tone rounded-2xl border border-pale-ash/40 shadow-subtle overflow-hidden">
            ...
            <div className="border-b border-pale-ash/40 bg-pure-white px-6 py-4">
                ...
                <p className="text-xs font-medium text-inkwell-gray mt-0.5">Direct messaging boards synchronized across active viewing schedules</p>
            </div>

            <div className="flex-1 flex overflow-hidden">
                {/* Left Side: Conversational Channels Deck */}
                <div className="w-80 border-r border-pale-ash/40 bg-cloud-whisper/40 overflow-y-auto hidden md:block">
                    <div className="p-4 space-y-3">
                        <h2 className="text-xs font-bold text-inkwell-gray uppercase tracking-wider px-1">Active Chat Threads</h2>
                                    ...
                                    <div className="flex justify-between items-start gap-2">
                                        <div className="flex-1 min-w-0 space-y-0.5">
                                            <p className="font-bold text-charcoal-tone text-sm truncate">
                                                {req.tenant?.full_name || 'Prospect Tenant'}
                                            </p>
                ...
                {/* Right Side: Active Room Interface Screen */}
                <div className="flex-1 flex flex-col bg-pure-white p-4">
                    {activeRequest && user ? (
                        <ChatWindow
                            viewingRequestId={activeRequest.id}
                            currentUserId={user.id}
                            otherPartyName={activeRequest.tenant?.full_name || 'Tenant'}
                        ...
                        <div className="flex-1 flex items-center justify-center text-center p-8 bg-cloud-whisper/20 rounded-2xl border border-dashed border-pale-ash/40 m-2">
                            <div className="space-y-2">
                                ...
                                <p className="text-xs font-bold text-stone-slate">No active interaction</p>
                                <p className="text-[11px] font-medium text-stone-slate/60 max-w-xs">Select a corresponding tenant thread card from the index array to open your active message canvas.</p>
                            </div>
                        </div>
                    )}
                </div>
